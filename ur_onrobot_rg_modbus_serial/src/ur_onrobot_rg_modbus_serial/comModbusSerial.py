#!/usr/bin/env python3
"""
Module comModbusSerial: defines a class which communicates with
OnRobot Grippers using the Modbus Serial protocol.
"""
...
class communication:
    """ communication sends commands and receives the status of RG gripper.
        ...
        Attributes:
            client (pymodbus.client.sync.ModbusSerialClient):
                instance of ModbusSerialClient to establish modbus connection
    ...
    def connectToDevice(self, device, changer_addr=65):
        """ Connects to the client device (gripper).

            Args:
                device (str): serial device (e.g. '/tmp/ttyUR')
